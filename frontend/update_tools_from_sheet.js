...
async function exportSheetToJS() {
  try {
    ...
    // Create the JS file content with the tools data
    const jsContent = `// Generated by export-sheet-to-json.js
// Last updated: ${new Date().toISOString()}

export const TOOL_DATA = ${JSON.stringify(tools, null, 2)};`;

    // Write to the JS file in src/app/utils/toolData.js
    const jsFilePath = path.join(process.cwd(), 'src', 'app', 'utils', 'toolData.js');
    fs.writeFileSync(jsFilePath, jsContent);
    console.log(`Data successfully exported to ${jsFilePath}`);

    // For backward compatibility, we'll also save the JSON file
    ...
    console.log('Data also saved to src/data/tools.json for backward compatibility');
